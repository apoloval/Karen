...
public:
   ...
   /**
    * Swap the nodes pointed by given linked lists.
    */
   inline static void swap(LinkedList* l1, LinkedList* l2)
   {
      LinkedList* prev1 = l1->prev(),
                 *next1 = l1->next(),
                 *prev2 = l2->prev(),
                 *next2 = l2->next();
      
      l1->_prev = prev2;  if (prev2) prev2->_next = l1;      
      l1->_next = next2;  if (next2) next2->_prev = l1;      
      l2->_prev = prev1;  if (prev1) prev1->_next = l2;
      l2->_next = next1;  if (next1) next1->_prev = l2;
   }
...
public:
   ...
   /**
    * Move the element pointed by given iterator to list head. If the element
    * is already the list head, nothing is done.
    */
   inline void moveToHead(Iterator& it)
   {
      LinkedList<T>* n = iteratorToNode(it);
      if (n != _first)
      {
         LinkedList<T>::swap(n, _first);
         it = Iterator(new ListIterator(this, _first));
         _first = n;
      }
   }
   
   /**
    * Move the element pointed by given iterator to list tail. If the element
    * is already the list tail, nothing is done.
    */
   inline void moveToTail(Iterator& it)
   {
      LinkedList<T>* n = iteratorToNode(it);
      if (n != _last)
      {
         LinkedList<T>::swap(n, _last);
         it._node = _last;
         _last = n;
      }
   }
